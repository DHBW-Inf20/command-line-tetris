...
using namespace std; //Nicht schön, wollten es aber quellengetreu übernehmen
...
int key_press() { // not working: F11 (-122, toggles fullscreen)
    ...
    while(true) {
        ...
        if(irec.EventType==KEY_EVENT&&((KEY_EVENT_RECORD&)irec.Event).bKeyDown) {
            ...
            const auto ca = (int)keyevent.uChar.AsciiChar;
            const auto cv = (int)keyevent.wVirtualKeyCode;
            const auto key = ca==0 ? -cv : ca+(ca>0?0:256);
            switch(key) {
